...
def y_axis_angular_vel_from_marix(matrix: torch.Tensor) -> torch.Tensor:
    """
    Calculate angular velocities from rotation matrices
    Args:
        mxtrix: batch of rotation matrices of size (*, 3, 3)

    Returns:
        y axis angular velocities of size (*, 1)
    """
    y_angle = torch.atan2(-matrix[:, 2, 0], torch.sqrt(torch.square(matrix[:, 2, 1]) + torch.square(matrix[:, 2, 2])))
    y_angular_vel = y_angle[1:] - y_angle[:-1]

    return y_angular_vel