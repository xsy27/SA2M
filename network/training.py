    ...
    def diffuse(self, x_start, t, cond, data_config, noise=None, return_loss=False):
        ...
        if return_loss:
            ...
            if self.config.trainer.use_loss_delta:
                ...
                loss_terms['loss_data_delta'] = self.diffusion.masked_l2(target_vel[:, :-1], model_output_vel[:, :-1], mask[..., 1:])
    ...
    def evaluate(self, dataloader, save_folder_name):
        ...
        with torch.no_grad():
            ...
            for datas in dataloader:
                ...
                model_output, losses = self.diffuse(x_start, t, cond, data_config, noise=None, return_loss=True)
